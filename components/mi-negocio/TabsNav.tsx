  ...
  return (
    <div className="flex gap-1 bg-white border border-slate-200 rounded-2xl p-1 shadow-sm overflow-x-auto">
      {TABS.map(({ href, label, icon }) => {
        const active = pathname === href || (href !== "/mi-negocio/bot" && pathname.startsWith(href));
        return (
          <Link
            key={href}
            href={href}
            className={`flex-1 min-w-fit flex items-center justify-center gap-1.5 px-3 py-2.5 rounded-xl text-sm font-medium transition-all whitespace-nowrap ${
              active
