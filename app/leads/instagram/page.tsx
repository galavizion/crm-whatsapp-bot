  ...
  if (isGod) {
    ...
    const [{ data: biz }, { data: leads }, { data: comments }] = await Promise.all([
      ...
      admin.from("contactos")
        ...
        .eq("canal", "instagram").order("ultima_respuesta", { ascending: false }),
      admin.from("social_comments")
        .select("id, comment_id, author_id, text, bot_reply, sent_dm, created_at, business_id")
        .eq("platform", "instagram").order("created_at", { ascending: false }).limit(200),
    ]);
    ...
    const [leadsQ, commentsQ] = await Promise.all([
      (() => {
        let q = supabase.from("contactos")
          ...
          .eq("business_id", bu.business_id).eq("canal", "instagram")
          .order("ultima_respuesta", { ascending: false });
        if (role !== "admin") q = q.eq("assigned_user_id", user.id);
        return q;
      })(),
      admin.from("social_comments")
        .select("id, comment_id, author_id, text, bot_reply, sent_dm, created_at")
        .eq("business_id", bu.business_id).eq("platform", "instagram")
    ...
    <main className="min-h-screen bg-neutral-50">
      <div className="px-4 py-6 md:px-6 lg:px-8 max-w-5xl">
        <div className="mb-6">
          ...
          <h1 className="text-2xl font-bold tracking-tight text-neutral-900">Instagram</h1>
          <p className="mt-1 text-sm text-neutral-600">DMs y comentarios de publicaciones.</p>
        </div>
        ...
        <div className="flex gap-1 mb-6 bg-white border border-neutral-200 rounded-2xl p-1 w-fit shadow-sm">
          <Link
            ...
            className={`inline-flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-medium transition ${activeTab === "dms" ? "bg-pink-500 text-white shadow-sm" : "text-neutral-600 hover:bg-neutral-100"}`}
          >
          ...
          <Link
            href="/leads/instagram?tab=comentarios"
            className={`inline-flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-medium transition ${activeTab === "comentarios" ? "bg-pink-500 text-white shadow-sm" : "text-neutral-600 hover:bg-neutral-100"}`}
          >
        ...
        {activeTab === "dms" && (
          contactos.length === 0 ? (
            <EmptyState text="No hay DMs de Instagram todavía." />
          ) : (
                  ...
                  <div className="space-y-2">
                    ...
                    {c.bot_reply && (
                      <div className="bg-pink-50 rounded-xl px-3 py-2.5">
                        <p className="text-[10px] font-semibold text-pink-400 uppercase tracking-wide mb-1">Respuesta del bot</p>
                        <p className="text-sm text-pink-900">{c.bot_reply}</p>
                      </div>
                    )}
                  </div>
                </div>
